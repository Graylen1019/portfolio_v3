...
export const ProjectPage = async (props: Props) => {
  const { "project.name": projectName } = await props.params;
  const projects = await getProjects();
  const project = projects.find((p) => toSlug(p.title) === projectName);

  if (!project) notFound();

  const metaKey = Object.keys(projectMeta).find(
    (key) => toSlug(key) === projectName
  );
  const meta = metaKey ? projectMeta[metaKey] : null;
    ...
    <main className="max-w-7xl mx-auto px-6 py-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-12 items-start">
        ...
        <div className="lg:sticky lg:top-24 flex flex-col gap-6">
          <div>
            <h1
              className="text-[clamp(24px,2.8vw,40px)] font-medium tracking-tight mb-4 text-(--text) capitalize"
          ...
          <div className="rounded-[14px] border border-(--line) overflow-hidden bg-[#0e0e12]">
          ...
          {stack.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {stack.map((item: { name: string; reason: string }, i: number) => (
                  <div
                    key={i}
          ...
          {challenges.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-4">
                {challenges.map((c: { title: string; problem: string; solution: string }, i: number) => (
                  <div
                    key={i}
                    className="border border-(--line) rounded-xl overflow-hidden"
                  >
                    <div className="px-5 py-3 border-b border-(--line) bg-(--surface) flex items-center gap-3">
                      <span className="font-mono text-[11px] text-(--dim) tracking-[0.16em]">
